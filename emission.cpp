...
#define FINAL_SUBSIDY                                   ((uint64_t)0) // 0 * pow(10, 0)
#define GENESIS_BLOCK_REWARD                            ((uint64_t)12600000000000)
#define INSTAMINE_BLOCK_REWARD                          ((uint64_t)12600000000000)
...
int main()
{
  const int target_minutes = DIFFICULTY_TARGET/2;
  ...
  while (coins_already_generated < FORK_MONEY_SUPPLY){
    bool emission_speed_change_happened = false;
    if (height > 239924 && height % COIN_EMISSION_HEIGHT_INTERVAL == 0){
      if (height < (PEAK_COIN_EMISSION_HEIGHT + COIN_EMISSION_HEIGHT_INTERVAL)){
        ...
        cal_block_reward = ((uint64_t)(FORK_MONEY_SUPPLY * money_supply_percentage)) >> EMISSION_SPEED_FACTOR;
      }
      else{
        cal_block_reward = ((uint64_t)(FORK_MONEY_SUPPLY - coins_already_generated)) >> EMISSION_SPEED_FACTOR;
      }
    ...
    if (emission_speed_change_happened && (print_by_year ? count % 2 : true)){
      if (height == 0){
        std::cout << height << "\t"
          << std::fixed << std::setprecision(2) << block_reward / 100.0 << "\t"
          << std::setprecision(8) << coins_already_generated / 100.0 << "\t"
        ...
        std::cout << height << "\t"
          << std::fixed << std::setprecision(2) << block_reward / 100.0 << "\t"
          << std::setprecision(2) << coins_already_generated / 100.0 << "\t"
          << std::fixed << std::setprecision(2) << coins_already_generated*100.0 / MONEY_SUPPLY << "\t"
          << std::fixed << std::setprecision(0) << total_time / (60 * 60 * 24.0) << "\t"
          << std::setprecision(2) << total_time / (60 * 60 * 24.0) / 365.25 << "\n";
      }
