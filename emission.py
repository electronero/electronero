...
FORK_MONEY_SUPPLY         = 21000000000000
GENESIS_BLOCK_REWARD     = 12600000000000
INSTAMINE_BLOCK_REWARD     = 12600000000000
FINAL_SUBSIDY = 0
...
PEAK_COIN_EMISSION_YEAR = 2
PEAK_COIN_EMISSION_HEIGHT = HEIGHT_PER_YEAR * PEAK_COIN_EMISSION_YEAR


def get_block_reward(height, coins_already_generated):
    target_minutes = DIFFICULTY_TARGET/2
    emission_speed_factor = EMISSION_SPEED_FACTOR - (target_minutes-1)
    if height < (PEAK_COIN_EMISSION_HEIGHT + COIN_EMISSION_HEIGHT_INTERVAL):
        ...
        cal_block_reward = (MONEY_SUPPLY - coins_already_generated)/(EMISSION_SPEED_FACTOR-1)
...
def calculate_emssion_speed(print_by_year = False):
    ...
    while coins_already_generated < MONEY_SUPPLY - FINAL_SUBSIDY:
        ...
        if height == 239925:
            block_reward = INSTAMINE_BLOCK_REWARD
        if height < 239924:
            block_reward = int(cal_block_reward)
        else:
        ...
        if emission_speed_change_happened and (count % 2 if print_by_year else True):
            print format(height, '07'), "\t", '{0:.10f}'.format(block_reward/100.0), "\t", coins_already_generated/100.0, "\t", str(round(coins_already_generated*100.0/MONEY_SUPPLY, 2)), "\t\t", format(int(total_time/(60*60*24.0)), '04'), "\t", total_time/(60*60*24)/365.25
            f.write(format(height, '07') + "\t" + '{0:.8f}'.format(block_reward/100.0) + "\t" + str(coins_already_generated/100.0) + "\t" + '%05.2f'%(coins_already_generated*100.0/MONEY_SUPPLY) + "\t" + format(int(total_time/(60*60*24.0)), '04') + "\t" + str(round(total_time/(60*60*24)/365.25, 2)) + "\n")
        
        height += 1
        
    print format(height, '07'), "\t", '{0:.10f}'.format(block_reward/100.0), "\t", coins_already_generated/100.0, "\t", str(round(coins_already_generated*100.0/MONEY_SUPPLY, 2)), "\t\t", format(int(total_time/(60*60*24.0)), '04'), "\t", total_time/(60*60*24)/365.
    f.write(format(height, '07') + "\t" + '{0:.8f}'.format(block_reward/100.0) + "\t" + str(coins_already_generated/100.0) + "\t" + '{0:.2f}'.format(round(coins_already_generated*100.0/MONEY_SUPPLY, 2)) + "\t" + format(int(total_time/(60*60*24.0)), '04') + "\t" + str(round(total_time/(60*60*24)/365.25, 2)) + "\n")
